...
export default FinanceSection;import React from 'react';
...
const FinanceSection: React.FC<FinanceSectionProps> = ({
  ...
  return (
    <div>
      ...
      <p className="mb-4">Account Balance: ${accountBalance.toFixed(2)}</p>
        ...
        <ul>
          {expenses.slice(0, 5).map((expense) => (
            <li key={expense.id} className="mb-2">
              ${expense.amount} - {new Date(expense.date).toLocaleDateString()} - {expense.description}
            </li>
          ))}
        </ul>
      </div>
